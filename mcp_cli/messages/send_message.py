...
async def send_message(
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            logging.debug(f"Attempt {attempt}/{retries}: Sending message: {message_dict}")
            await write_stream.send(message_dict)
                ...
                async for response in read_stream:
                    if not isinstance(response, Exception):
                        if isinstance(response, JSONRPCMessage):
                            ...
                            response_dict = {"error": f"Invalid response type: {type(response)}"}
                        
                        logging.debug(f"Received response: {response_dict}")
